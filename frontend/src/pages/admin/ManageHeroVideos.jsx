...
// Brand Colors - Used automatically (Supervisor cannot change these)
const BRAND = {
  navy: '#1A3258',
  maroon: '#A53D32',
  gold: '#B69F60',
...
const ManageHeroVideos = () => {
  ...
  const badgeOptions = ['Live Now', 'Featured', 'Premium', 'New Release', 'Trending', 'Exclusive', ''];
  ...
  const loadVideos = async () => {
    ...
    try {
      ...
      console.error('Load hero videos error:', error);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Fixed colors - automatically applied
      const payload = {
      ...
      let response;
      if (editingVideo) {
        response = await api.put(`/admin/hero/${editingVideo._id}`, payload);
      } else {
        response = await api.post('/admin/hero', payload);
      }
      
      if (response.data.success) {
        toast.success(editingVideo ? 'Hero video updated successfully' : 'Hero video added successfully');
        setShowForm(false);
      ...
      toast.error(error.response?.data?.error || 'Failed to save hero video');
    } finally {
  ...
  const handleDelete = async (video) => {
    if (!window.confirm(`Are you sure you want to delete "${video.title}" from the main board?`)) return;
    
    try {
      const response = await api.delete(`/admin/hero/${video._id}`);
      if (response.data.success) {
  ...
  const handleToggleActive = async (video) => {
    try {
      const response = await api.put(`/admin/hero/${video._id}/toggle-active`);
      if (response.data.success) {
        toast.success(`Hero video ${video.isActive ? 'deactivated' : 'activated'}`);
        loadVideos();
      }
    } catch (error) {
  ...
  return (
    <>
      <Helmet>
        <title>Manage Main Board - TradeExTV</title>
      </Helmet>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className={`text-sm mt-1 ${isDark ? 'text-emerald-200/50' : 'text-emerald-800/50'}`}>
              Manage videos displayed on the homepage hero banner
            </p>
          </div>
          <button
            onClick={() => {
            ...
            className="px-6 py-2.5 bg-gradient-to-r from-amber-500 to-orange-600 text-white font-semibold rounded-xl transition-all duration-300 hover:scale-[1.02] hover:shadow-xl shadow-lg shadow-amber-500/20 flex items-center gap-2"
          >
            <FiPlus size={18} />
            Add to Main Board
          </button>
        ...
        <div className={`p-4 rounded-2xl ${
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                type="text"
                placeholder="Search main board videos..."
                value={searchTerm}
        ...
        {/* Add/Edit Form - NO COLOR OPTIONS */}
        {showForm && (
          <div className={`p-6 rounded-2xl animate-slide-down ${
            ...
            <h3 className={`text-lg font-semibold mb-4 ${isDark ? 'text-white' : 'text-emerald-950'}`}>
              {editingVideo ? 'Edit Main Board Video' : 'Add to Main Board'}
            </h3>
            
            {/* Color Info Banner - Shows that colors are automatic */}
            <div className={`mb-4 p-3 rounded-xl text-xs flex items-center gap-2 ${
              ...
              <span>Colors are automatically applied using brand colors (Navy, Maroon, Gold)</span>
            </div>
            ...
            <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-4">
              ...
              {/* Color Preview - Shows what colors will be used (read-only) */}
              <div className="md:col-span-2">
                <label className={`block text-sm font-medium mb-2 ${
                  isDark ? 'text-emerald-200/70' : 'text-emerald-800/70'
                }`}>
                  Color Preview (Auto-Applied)
                </label>
                <div className="flex gap-3">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded" style={{ backgroundColor: BRAND.navy }} />
                    <span className={`text-xs ${isDark ? 'text-emerald-200/60' : 'text-emerald-800/60'}`}>Navy</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded" style={{ backgroundColor: BRAND.maroon }} />
                    <span className={`text-xs ${isDark ? 'text-emerald-200/60' : 'text-emerald-800/60'}`}>Maroon</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded" style={{ backgroundColor: BRAND.gold }} />
                    <span className={`text-xs ${isDark ? 'text-emerald-200/60' : 'text-emerald-800/60'}`}>Gold</span>
                  </div>
                </div>
              </div>
              ...
              <div className="md:col-span-2 flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-3 bg-gradient-to-r from-amber-500 to-orange-600 text-white font-semibold rounded-xl transition-all duration-300 hover:scale-[1.02] hover:shadow-xl shadow-lg shadow-amber-500/20 flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <>
                    ...
                    <>
                      <FiSave size={18} />
                      {editingVideo ? 'Update Main Board Video' : 'Add to Main Board'}
                    </>
                  )}
                </button>
                <button
          ...
          {filteredVideos.length === 0 ? (
            <div className="col-span-full p-12 text-center rounded-2xl">
              ...
              <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-emerald-950'}`}>
                No Main Board Videos
              </h3>
              <p className={`text-sm mt-1 ${isDark ? 'text-emerald-200/50' : 'text-emerald-800/50'}`}>
                {searchTerm ? 'Try a different search term' : 'Add videos to the main board hero banner'}
              </p>
            </div>
          ) : (
            filteredVideos.map((video) => (
              <div
                ...
                className={`rounded-2xl overflow-hidden border transition-all duration-300 ${
                  isDark
                    ? 'bg-[#032e1d]/40 border-white/5 hover:border-amber-500/30'
                    : 'bg-white/60 border-emerald-100/50 hover:border-amber-400/30'
                } backdrop-blur-sm`}
              >
                {/* Thumbnail */}
          ...
          <div className={`p-4 rounded-2xl ${
            ...
            <div className="flex flex-wrap items-center gap-6 text-sm">
              <span className={isDark ? 'text-emerald-200/60' : 'text-emerald-800/60'}>
                Total Main Board Videos: <span className={`font-bold ${isDark ? 'text-white' : 'text-emerald-950'}`}>
                  {videos.length}
                </span>
              </span>
              <span className={isDark ? 'text-emerald-200/60' : 'text-emerald-800/60'}>
...
export default ManageHeroVideos;